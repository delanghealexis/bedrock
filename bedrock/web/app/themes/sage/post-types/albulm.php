...
/**
 * Registers the `albulm` post type.
 */
function albulm_init() {
	register_post_type(
		'albulm',
		[
			'labels'                => [
				...
				'featured_image'        => _x( 'Featured Image', 'albulm', 'sage' ),
				'set_featured_image'    => _x( 'Set featured image', 'albulm', 'sage' ),
				'remove_featured_image' => _x( 'Remove featured image', 'albulm', 'sage' ),
				'use_featured_image'    => _x( 'Use as featured image', 'albulm', 'sage' ),
...
add_action( 'init', 'albulm_init' );

/**
 * Sets the post updated messages for the `albulm` post type.
 *
 * @param  array $messages Post updated messages.
 * @return array Messages for the `albulm` post type.
 */
function albulm_updated_messages( $messages ) {
	global $post;
...
add_filter( 'post_updated_messages', 'albulm_updated_messages' );

/**
 * Sets the bulk post updated messages for the `albulm` post type.
 *
 * @param  array $bulk_messages Arrays of messages, each keyed by the corresponding post type. Messages are
 *                              keyed with 'updated', 'locked', 'deleted', 'trashed', and 'untrashed'.
 * @param  int[] $bulk_counts   Array of item counts for each message, used to build internationalized strings.
 * @return array Bulk messages for the `albulm` post type.
 */
function albulm_bulk_updated_messages( $bulk_messages, $bulk_counts ) {
	global $post;

	$bulk_messages['albulm'] = [
...
add_filter( 'bulk_post_updated_messages', 'albulm_bulk_updated_messages', 10, 2 );
